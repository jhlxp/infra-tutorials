...
RDMA 的核心逻辑，就是把"派活"和"干活"彻底拆开：干活交给专门的搬砖工（DMA 引擎），派活则交给一套叫 QP/WQE 的硬件抽象。一句话理解 RDMA 的本质：**用硬件抽象替代软件协议栈，让 DMA 引擎直接搬数据，把 CPU 从搬砖的苦差事里彻底解放出来。**
...
-   **网卡 DMA**：网卡内置 DMA 引擎负责 GPU 显存与网络之间的搬运，是 GPUDirect RDMA 的底层
