...
> **适用范围**：文中硬件参数以 H100/B200/B300 + ConnectX\-7/8 为主要参考平台；NCCL 版本特性截至 2.29、NVSHMEM 截至 3.5。所有数字请以你自己的 benchmark 和 NVIDIA 官方 Release Notes 为准。
...
-   **NVLink SHARP / NVLS**：**NVLink 域内**的交换层硬件归约能力。注意它与机间 InfiniBand 网络里的 SHARP 不是同一层能力，不能混为一谈。NCCL 自动探测并可通过 `NCCL_NVLS_ENABLE` 控制。
...
跨机后带宽从 NVLink（TB/s 级）落到单 NIC（~50 GB/s）量级，约 **18×** 落差——厘米级板级链路 vs 米–百米级光纤/铜缆 + NIC/PCIe 开销。IB / RoCE 选型 ≈ **硬件保障** vs **运维调参**。
