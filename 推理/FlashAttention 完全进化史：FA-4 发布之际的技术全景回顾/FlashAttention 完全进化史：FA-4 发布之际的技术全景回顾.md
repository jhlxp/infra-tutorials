...
假设我们在训练 GPT-3 规模的模型，序列长度 2048，hidden dimension 12288，96 个注意力头，那么每个头的 head\_dim 是 128。我们把注意力机制拆开看：
...
A100 的算力峰值是 312 TFLOPs/s（FP16 Tensor Core），HBM 带宽约 1.6 TB/s（40GB）/ 2.0 TB/s（80GB）。我们算一下这个 kernel 的算术强度：
...
看起来还不错？但问题在于，S 与 P 两个各约 16 MB 的矩阵必须完整地写入和读取 HBM。我们看 Roofline 模型：
...
这就是 FlashAttention 要解决的核心问题：如何在分块计算的情况下，仍然得到正确的 softmax 结果，且不需要回头修改已经处理过的数据。
