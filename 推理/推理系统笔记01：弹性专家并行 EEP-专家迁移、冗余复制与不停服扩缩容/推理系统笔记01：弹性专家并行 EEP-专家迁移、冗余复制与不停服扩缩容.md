...
> MoE 大模型有几百个专家（Expert），单卡放不下，只能分散到多张 GPU 上。最朴素的做法是均匀平摊——听起来很美。但真实流量里，某几个专家会突然变"热"，承载它的那张卡被打爆，其余卡却闲等。本篇从这个矛盾出发，讲清楚 EEP（Elastic Expert Parallelism，弹性专家并行）如何在运行时把负载重新摊平：专家迁移怎么搬权重、为什么光搬不够还要复制、vLLM/SGLang 真实系统用什么通信原语、动态扩缩容为什么落在 DP 维度，以及它和 PD 分离扩缩容是什么关系。
...
vLLM 的 Elastic EP 有个值得理解的设计选择：**弹性维度放在数据并行（DP），而不是张量并行（TP）。** 核心是一个等式：
