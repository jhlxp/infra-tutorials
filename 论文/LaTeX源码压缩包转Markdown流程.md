...
这个流程的核心产物是“论文原文的中文译文”，不是论文摘要、学习笔记，也不是简单把 LaTeX 转成 Markdown。工具链能解决的是结构展开、图片转换、引用编号和排版，但真正决定质量的是逐段翻译。

翻译时遵守这些原则：

1. 按原论文段落顺序翻译，尽量保持原文的信息密度、论证顺序和技术细节。
2. 不要把正文改写成提纲式总结；如果原文是一段论证，译文也应该是一段论证。
3. 术语第一次出现时可以保留英文，例如“负载均衡（load balancing）”“包喷洒（packet spraying）”，后文再统一使用中文或中英混排。
4. 不确定的专有名词、系统名、算法名、数据集名不要硬翻，优先保留英文。
5. 数学符号、变量名、图号、表号、citation 编号必须跟随原文保留，不能在翻译时丢掉。
6. 如果原文一句很长，可以拆成两句中文，但不能改变因果关系、限定条件和结论强弱。
7. 翻译完成后要二次通读：检查是否有机器翻译腔、断句错误、术语前后不一致、引用编号丢失、公式附近语义不通。

推荐工作方式：

1. 先完成结构化转换：章节、图表、公式、参考文献编号都到位。
2. 再逐节翻译正文，而不是一次性粗翻全篇。
3. 每翻译一节，就对照原文检查技术含义。
4. 最后整体润色，让中文能顺畅阅读，但不要为了顺畅而省略原文信息。
