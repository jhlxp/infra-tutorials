...
DeepEP 的核心是两个 CUDA Kernel 函数——dispatch 和 combine，分别对应 MoE 模型专家层中 token 分发给 experts 和聚合回原 rank 的两次 All-to-All 通信（见图 1）。  
...
DeepEP 的 dispatch kernel 采用两阶段设计（send/recv），其中 send 阶段还支持开启 FP8 量化。开启 FP8 量化可将原始 BF16 格式的 token 向量转换为 FP8 格式，数据量减少 50%。为保持精度，增加 scales 进行量化缩放，通过范围缩放确保数据分布落在 FP8 类型的有效表示范围内。  
...
DeepEP 的 low latency kernel 运行时会尽可能多的占用 sm，以提高通信效率。在 token data send 阶段，每个循环迭代并行处理 num\_sms 个 tokens，首先对需要量化的 token 执行 FP8 转换，然后每个 sm 使用前 topk（通常为 8）个 warps 将 token 分发给 8 个目标 experts，调用 warp 级的 `nvshmemi_ibgda_put_nbi_warp` 进行 RDMA 传输。
