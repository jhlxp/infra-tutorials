...
**（1）ZeRO-1 的通讯量为什么是 $3\Phi$ 而不是 $2\Phi$？**
...
那么 $3\Phi$ 是怎么来的呢？因为论文定义 ZeRO-1 只有 optimizer 是切开的，意味着 G 和 W 都是完整的。所以对 G 做 all-reduce（虽然拿回完整的 G 并没有意义），对 W 做 all-gather，这样通讯量就是 $3\Phi$ 。

本文写作时，最终选择按照论文对相关概念的定义，选择了 $3\Phi$ ，但是实操来看是完全可以用 $2\Phi$ 实现的。评论区有朋友提到 DeepSpeed 的某次代码更新是将 ZeRO-1 的通讯量从 $3\Phi$ 降至 $2\Phi$ ，可能也是基于此做了改进。
...
在混合精度训练的前提下，这边我们对 **【只能将相应的W（蓝色部分）进行更新】** 这句话做更严谨的说明。
