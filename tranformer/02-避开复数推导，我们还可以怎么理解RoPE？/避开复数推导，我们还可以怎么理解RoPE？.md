...
transformer位置编码原理我们在[这篇文章](https://zhuanlan.zhihu.com/p/454482273)中详细讲过，这里我们对它进行一个快速的回顾解读。了表达方便，我们先**以二维特征空间为例，根据transformer PE的构造方法，我们有**：  
...
**这是因为，我们上述的一切分析，都是在原始位置编码** $PE_{t}^{T} * PE_{t + \Delta t}$ **上的结果**，我们一般把位置编码和输入层的token相加，然后让他们继续去做接下来的计算。可是，transformer架构是复杂的，更详细地说，**当token向量进入attention层时，起作用的还是** $PE_{t}^{T} * PE_{t + \Delta t}$ **这个部分吗？**  
...
从中我们不难发现，经过attention层后，位置编码真正起作用的不再是 $PE_{t}^{T} * PE_{t + \Delta t}$ ，而是引入了线性变化后的 $PE_{t}^{T}W_{Q}^{T}W_{K}PE_{t + \Delta t}$ 。**那么再引入这种线性变化后，位置编码还能保持上述所说的绝对性、相对性和远距离衰减性这种优良性值吗？我们同样用实验的方式来细看这一点。**  
...
总结起来，在二维特征空间下，我们定义了一个**逆时针正交旋转矩阵(正如1.1所说，正交旋转矩阵保证了模长不变，即维护q，k原始的特征，只单纯做旋转使其拥有绝对位置信息)：**  
